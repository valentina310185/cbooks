...
class SystemQueries
{
	...
	public static function GetFormInputsDefinition($form_name_id)
	{
		$form_input_query = "
			select 
			`input_form_id`, 
	...
	public static function GetFormInputsFileType($form_name_id, $input_name)
	{
		$form_input_query = "
			select 
			`input_form_id`, 
	...
	public static function GetViewColumns($table_view_name)
	{
		$query = "
			...
			`table_view_name`, 
			`table_view_column_name`, 
			`table_view_column_header_label`,
			`table_view_column_order_number`, 
			`table_view_column_is_hidden`, 
			...
			WHERE 
			table_view_name =  '" . $table_view_name . "'
			and table_view_column_is_hidden = 'NO'
	...
	public static function form_input_structure($form_name_id)
	{
		$query = "
			SELECT 
			`input_form_id`, 
			...
			`lazy_inputs_validations` as iv
			WHERE
			fi.input_validation_name = iv.input_validation_name
			and fi.input_is_hidden = 'NO'
			and fi.form_name_id = '" . $form_name_id . "';
		";
		return DBManager::ExecuteQuery($query);
	}
